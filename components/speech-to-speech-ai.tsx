...
// import { SpeechRecognition } from '@/types/speech-recognition.types'
...
export function SpeechToSpeechAiOld() {
  ...
  const handleSubmit = async () => {
    ...
    const aiResponse = await new Promise((resolve) =>
      setTimeout(
        () =>
          resolve(
            "This is a simulated AI response. In a real application, this would be the response from the OpenAI API."
          ),
        2000
      )
    );
    setIsLoading(false);
    speakResponse(aiResponse as string);
  };
  ...
  const speakResponse = (text: string) => {
    if (synthRef.current) {
      ...
      utterance.onend = () => {
        ...
        setMessages((prev) => [
          ...prev,
          { id: Date.now().toString(), type: "ai", text, audio: audioURL },
        ]);
      };
